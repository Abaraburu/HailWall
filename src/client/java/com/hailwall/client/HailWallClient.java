...
/**
 * Client-side entrypoint. Answers the server's login query by collecting every
 * top-level installed mod (id + version), signing the list, and sending it back.
 *
 * <p>Only top-level mods are reported: nested (jar-in-jar) library mods are
 * skipped, so a whitelist stays manageable. The parent mod that bundles them is
 * still reported and checked.</p>
 *
 * <p>It also prints the reported modlist to the client's own log
 * ({@code .minecraft/logs/latest.log}) so admins can copy the modids easily.</p>
 */
public class HailWallClient implements ClientModInitializer {
	private static final Logger LOGGER = LoggerFactory.getLogger("HailWall");

	@Override
	public void onInitializeClient() {
		ClientLoginNetworking.registerGlobalReceiver(Protocol.CHANNEL, (client, listener, buf, callbacksConsumer) -> {
