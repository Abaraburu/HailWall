...
/**
 * Fabric server-side entrypoint for Minecraft 1.21.1. During the login phase it asks
 ...
 *
 * <p>All the loader-agnostic logic lives in the shared {@code core} (config, codec,
 * signing, verifier, access log); this class only provides the Fabric "glue":
 * the login-networking handshake and the mod enumeration is done client-side.</p>
 */
public class HailWallFabric implements ModInitializer {
	...
	/** Login channel as a 1.21.1 {@link ResourceLocation}, built from the loader-agnostic Protocol strings. */
	...
	/** Ids always permitted on Fabric, even in whitelist mode. */
	...
	/** Id prefixes always permitted (fabric-api ships many "fabric-*" submodule ids). */
	 ...
	 * via an accessor because {@code getUserName()} can return a decorated string
	 * that would not match operator names.
	 */
