...
/**
 * Fabric server-side entrypoint for Minecraft 1.16.5. Same logic as the other Fabric
 * targets, written Java-8 clean: disconnect messages use {@code new TextComponent(...)},
 * {@code ResourceLocation} via its constructor, and {@code Collections.emptyMap()} instead
 * of {@code Map.of()} (which is Java 9+).
 */
public class HailWallFabric implements ModInitializer {
	...
	private void handleNoCompanion(MinecraftServer server, ServerLoginPacketListenerImpl listener) {
		...
		if (!config.requireCompanionMod) {
			accessLog.record(who, true, "companion mod absent (not required)", Collections.<String, String>emptyMap());
			return;
		}
		if (!config.enforce) {
			LOGGER.info("[HailWall] (monitor) {} has no companion mod (not kicked).", who);
			accessLog.record(who, true, "MONITOR (no companion mod)", Collections.<String, String>emptyMap());
			return;
		}
		if (config.operatorsBypass && isOperator(server, who)) {
			LOGGER.info("[HailWall] {} is operator -> bypassing restrictions (allowed).", who);
			accessLog.record(who, true, "OP bypass (no companion mod)", Collections.<String, String>emptyMap());
			return;
		}
		LOGGER.info("[HailWall] Rejecting {}: no companion mod.", who);
		accessLog.record(who, false, "no companion mod", Collections.<String, String>emptyMap());
		listener.disconnect(new TextComponent(config.messageNoCompanion));
	}
	...
	private void handleResponse(MinecraftServer server, ServerLoginPacketListenerImpl listener,
		...
		String who = playerName(listener);
		Map<String, String> mods = malformed ? Collections.<String, String>emptyMap() : ModListCodec.decode(canonical);
