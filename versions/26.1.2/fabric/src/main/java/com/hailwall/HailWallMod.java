...
/**
 * Server-side entrypoint for Minecraft 26.1.2. During the login phase it asks the
 * client for its mod list, verifies the (signed) response against the config, and
...
public class HailWallMod implements ModInitializer {
	...
	/** Login channel as a 26.1.2 {@link Identifier}, built from the loader-agnostic Protocol strings. */
	static final Identifier CHANNEL =
			Identifier.fromNamespaceAndPath(Protocol.CHANNEL_NAMESPACE, Protocol.CHANNEL_PATH);
